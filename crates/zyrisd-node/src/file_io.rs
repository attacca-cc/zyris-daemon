//! Wraps `FileIo` to confine paths to the root.
//!
...
impl GatedFileIo {
    ...
    ///
    /// The layer below uses absolute paths as-is — the upstream contract — so no double resolve.
...
mod tests {
    ...
    async fn every_method_refuses_a_path_outside_the_root() {
        ...
        assert!(!Path::new("/etc/zyrisd-nope").exists());
    }
    ...
    /// deny applies to file_io too.
    #[tokio::test]
    async fn a_denied_subtree_is_refused_even_inside_the_root() {
        let dir = tempfile::tempdir().unwrap();
        let root = dir.path().canonicalize().unwrap();
        std::fs::create_dir(root.join("keys")).unwrap();
        std::fs::write(root.join("keys/id"), b"secret").unwrap();
        let g = GatedFileIo::new(PathGate::new(vec![root.clone()], vec![root.join("keys")]));
        assert!(g.read("keys/id".into(), None, None).await.is_err());
    }
}
