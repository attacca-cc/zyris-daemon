...
impl PathGate {
    ...
    ///
    /// The announced tool description is a fixed string built by a macro, so the gate cannot
    /// change it, and it says "absolute paths are used as-is". This message is the only clue the
    /// model has to correct itself.
...
mod tests {
    ...
    fn an_absolute_path_outside_the_root_is_refused() {
        ...
        // The macro fixes the tool description and it says "absolute paths are used as-is",
        // so the model can only correct itself if the error names the allowed roots.
